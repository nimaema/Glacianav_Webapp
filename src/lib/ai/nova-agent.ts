  ...
  fn(
    ...
    "Compose your final answer as structured visual blocks — the chat renders each block as a real UI component (stat cells, entity cards, task rows, a colored callout, a tappable next-step chip). Call this ONCE, as your LAST tool call, for any substantive reading: workspace stats, account status, task overviews, plans, imports, anything with 3+ facts. Skip it for quick one-line facts and casual conversation. After calling it, reply with an empty message or one short closing line — never repeat the presented content as text.",
    {
      ...
      blocks: {
        type: "array",
        description:
          'The visual blocks, in reading order. Kinds: {kind:"stats", items:[{label, value, tone, delta?}]} for 2-6 key numbers (tone: teal|violet|rose|green|coral|gold|neutral — pick meaningfully: green=good, coral=problem, gold=watch); {kind:"bars", title?, items:[{label, value:number, display?, tone}]} for ONE measure distributed across 2-8 categories (customers per stage, tasks per owner, notes per segment) — rendered as proportional bars with the exact count; prefer bars over a table when there is only one number per category; {kind:"trend", label, points:[numbers oldest→newest], unit?, note?} for a REAL time series from tool results (2-24 points, e.g. interviews per week) — rendered as a sparkline; never fabricate points; {kind:"table", title?, columns:[{label, align?:"left"|"right"}], rows:[[cell strings]]} for 3+ records that share fields (customers, tasks, events — align:"right" for numeric columns; a cell may be prefixed "tone:" like "coral:overdue" to color it) — PREFER a table over entities/tasks whenever the records compare across the same 2-5 fields; {kind:"entities", title?, items:[{title, subtitle?, tone, meta?:[strings]}]} for records where a one-line story per item matters more than field comparison; {kind:"tasks", title?, items:[{text, done, who?, due?}]} for to-dos and done/pending reports; {kind:"timeline", title?, items:[{text, when?, tone, done?}]} for a dated sequence — account history, a plan\'s ordered steps, what happened then what\'s next (done:true = already happened); {kind:"quote", text, who?, source?, tone} for ONE verbatim customer quote from a validation note or transcript — the exact words, never paraphrased or invented (max one per answer); {kind:"callout", tone:"info"|"win"|"warn"|"risk", title?, body} for THE one insight or warning (max one per answer); {kind:"next", label, prompt} for the single obvious next move — label is what the user sees, prompt is the exact message tapping it sends back to you (max one per answer, put it last); {kind:"confirm", title, body?, confirmLabel?, confirmPrompt, cancelLabel?, cancelPrompt, tone} for a low-risk conversational yes/no decision. Both buttons send their exact prompt. Never use it in place of a signed destructive-action confirmation; {kind:"input", label, prompt, placeholder?, inputType?:"text"|"number"|"date", submitLabel?, initialValue?, min?, max?, step?} when you need ONE specific free value — text renders a field, number renders a stepper, and date renders the native date picker. "{value}" in prompt is replaced with the answer; {kind:"scale", label, prompt, display?:"slider"|"steps", min, max, step?, initialValue?, minLabel?, maxLabel?, submitLabel?, tone} for a bounded numeric answer. Use steps for a short 2-10 point rating or confidence scale and slider for a continuous-feeling range; {kind:"choice", title?, mode?:"single"|"multiple", allowCustom?:boolean, submitLabel?, minSelections?:number, maxSelections?:number, options:[{label, description?, prompt, tone}]} for an interactive decision. Use mode:"single" for one path and mode:"multiple" only when the options can genuinely be combined. For an exact count, set minSelections and maxSelections to the same number; for "up to N", set maxSelections; for "at least N", set minSelections. Multiple options render as a checklist with one submit action. Use choice for a bounded decision, input for a free value, scale for a bounded number, confirm for a binary low-risk decision, and make every resulting prompt executable.',
        items: {
          type: "object",
          properties: {
            kind: p("string", '"stats", "bars", "trend", "table", "entities", "tasks", "timeline", "quote", "callout", "next", "confirm", "input", "scale", or "choice".'),
            title: p("string", "Optional small section label (table/entities/tasks/bars/timeline) or callout title."),
            tone: p("string", "Callout tone (info, win, warn, risk) or a quote's tone."),
            body: p("string", "Callout body text."),
            label: p("string", "Next-move chip label, trend label, or input label."),
            prompt: p("string", 'Message sent back to you when tapped (next/choice/input — for input, "{value}" is replaced with what the user typed).'),
            text: p("string", "For a quote block: the verbatim quote text."),
            who: p("string", "For a quote block: who said it."),
            source: p("string", 'For a quote block: where it comes from (e.g. a recording title or "validation note").'),
            points: { type: "array", description: "For a trend block: the numeric series, oldest to newest.", items: { type: "number" } },
            unit: p("string", "For a trend block: optional unit shown after the latest value."),
            note: p("string", "For a trend block: optional one-line context (range, source)."),
            placeholder: p("string", "For an input block: placeholder text inside the field."),
            inputType: p("string", 'For an input block: "text" (default), "number", or "date".'),
            initialValue: { type: ["string", "number"], description: "For an input or scale block: an optional starting value. Use an ISO date string for a date input." },
            min: p("number", "For a number input or scale block: the minimum value."),
            max: p("number", "For a number input or scale block: the maximum value."),
            step: p("number", "For a number input or scale block: the positive increment."),
            display: p("string", 'For a scale block: "slider" (default) or "steps" for a short rating scale.'),
            minLabel: p("string", "For a scale block: concise meaning of the minimum."),
            maxLabel: p("string", "For a scale block: concise meaning of the maximum."),
            confirmLabel: p("string", "For a confirm block: concise positive-action label."),
            confirmPrompt: p("string", "For a confirm block: exact message sent by the positive action."),
            cancelLabel: p("string", "For a confirm block: concise decline label."),
            cancelPrompt: p("string", "For a confirm block: exact message sent by the decline action."),
            ...
            mode: p("string", 'For a "choice" block: "single" (default) or "multiple" when choices can be combined.'),
            allowCustom: p("boolean", 'For a "choice" block: show a route to write a custom reply. Default true.'),
            submitLabel: p("string", "Optional concise action label for a multiple-choice, input, or scale block."),
            minSelections: p("number", 'For a multiple-choice block: minimum selections required before submit. Set equal to maxSelections for an exact count.'),
            maxSelections: p("number", 'For a multiple-choice block: maximum selections allowed. Set equal to minSelections for an exact count.'),
            options: { type: "array", description: 'For a "choice" block: interactive options [{label, description?, prompt, tone}]. prompt is the exact message sent back for that option.', items: { type: "object" } },
...
function systemPrompt(ctx: Ctx, extraContext?: string): string {
  ...
  return [
    ...
    `- When data or a request doesn't fit a tool, don't force it into the wrong field or invent a capability. Do the unambiguous part, then ask via a present_answer {kind:"choice"} block. Use mode:"single" when the user must pick one path; use mode:"multiple" only when several options can be applied together. Offer 2-4 options that each map to a REAL capability (an account note, individual customers, tasks, a stage/next-step, or leave it out). Keep allowCustom true unless only the listed options are valid. For a single obvious next step instead, use a {kind:"next"} chip. Never write a prose menu or "Would you like me to…?".`,
    `- When a multiple-choice question requires an exact number, set minSelections and maxSelections to that number. For "up to N", set maxSelections; for "at least N", set minSelections. Let the control communicate the rule instead of repeating it in prose.`,
    `- Match the interaction to the answer: choice for named routes, input for one free text/number/date value, scale for a bounded quantity or rating, confirm for a low-risk binary decision, and next for one obvious continuation. Never use a conversational confirm block for deletion, spreadsheet writes, or another protected action — those require the signed Confirm control returned by the real tool.`,
    ...
    `- For any substantive reading (stats, account status, task overview, plan, import result — 3+ facts), finish with ONE present_answer call: a headline plus typed blocks the chat renders as real components (stat readouts, distribution bars, a trend sparkline, tables, entity rows, task rows with checkboxes, a timeline, one verbatim quote, one callout, and the right compact interaction: next move, confirm, input, scale, or choice).`,
    `- Prefer a {kind:"table"} whenever 3+ records share fields (customers by stage/owner/priority, tasks by account/due, events by time/who). Right-align numeric columns; tone-prefix status cells ("coral:overdue", "green:won"). Use entities only when each item's one-line story matters more than comparing fields. Use {kind:"bars"} instead of a table when comparing ONE count across categories, and {kind:"trend"} only for a real numeric series from tools.`,
    `- Ground every visual in evidence: a {kind:"quote"} is the customer's exact words from a note or transcript, never paraphrased. A {kind:"timeline"} orders what actually happened (done:true) against what's planned. When one missing value blocks you (a segment name, a due date), ask with {kind:"input"} instead of an open prose question.`,
    `- Use at most one interactive block in a normal answer so the next move stays obvious. When the user explicitly asks for an interaction gallery or UI demonstration, you may show one example of every interaction type in a single answer; make every example safe and non-destructive.`,
